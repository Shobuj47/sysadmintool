			...
			<c:if test="${userlist != null}">
				...
				<form:select class="form-control" path="userlist" id="userlist">
					<c:forEach var="user" items="${userlist}" varStatus="st">
						<c:if test="${ user.componentId == defaultusr }">
							<form:option value="${user.componentId}" selected="selected" label="${user.username}"/>
						</c:if>
						<form:option value="${user.componentId}" label="${user.username}"/>
					</c:forEach>
				</form:select>
				<input type="submit" onclick="getUserRoles()" value="Load" />
	...
	<div class="col-md-4 my-md-5">
			<c:if test="${userrolelist != null}">
				...
				<form:select class="form-control" id="newrolelist" path="rolelist" multiple="true">
					<c:forEach var="usrrolelist" items="${userrolelist}" varStatus="st">
						<form:option value="${usrrolelist.componentId}" label="${usrrolelist.displayname}"/>
					</c:forEach>
				</form:select>
			</c:if>
...
<script>
       ...
       selecteduser.addEventListener('change', function() {
    	   var usrId = selecteduser.options[selecteduser.selectedIndex].value;
    	   var usrName = selecteduser.options[selecteduser.selectedIndex].text;
    	   var url = window.location.href.split('?')[0];
    	   // Add that data to the <p>
    	   alert(url + "?usrId=" + usrId);
    	   window.location.href = url + "?usrId=" + usrId;
    	 })
